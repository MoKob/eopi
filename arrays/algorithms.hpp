...
// element less needs to allow comp(pivot,*itr) and comp(*itr,pivot)
...
iterator_type remove(iterator_type const begin, iterator_type const end,
  ...
  auto itr = begin;
  while (itr != end) {
    if (static_cast<key_type>(*itr) != key)
      *new_end++ = *itr;
    ++itr;
  }
...
// find the smallest positive integer missing in data.
// Destroys the content of data
inline std::int32_t smallest_missing_positive(std::vector<std::int32_t> &data) {
  for (std::size_t i = 0; i < data.size();) {
    if (data[i] != static_cast<std::int32_t>(i) &&
        data[i] < static_cast<std::int32_t>(data.size()) && data[i] >= 0) {
...
inline std::uint32_t max_difference(std::vector<std::int32_t> const &values) {
  ...
  for (auto val : values) {
    if (val < current_min)
      ...
      diff = std::max(diff, static_cast<std::uint32_t>(val - current_min));
  }
  return diff;
}
...
inline std::uint64_t
max_difference_twice(std::vector<std::int32_t> const &values) {
  if (values.empty())
    return 0;
  ...
  auto const best_diff_until = [&](auto const val) {
    ...
    return diff = std::max(diff, static_cast<std::uint32_t>(val - current_min));
  };
  ...
  for (std::size_t i = 0; i < values.size(); ++i) {
    ...
    if (val > current_max)
      ...
      diff = std::max(diff, static_cast<std::uint32_t>(current_max - val));
      total_best = std::max(total_best,
                            diff + best_forward[best_forward.size() - i - 1]);
    }
  }
...
inline std::uint64_t
max_difference_infinite(std::vector<std::int32_t> const &values) {
...
inline std::uint64_t max_difference(std::vector<std::int32_t> const &values,
  ...
  if (static_cast<std::size_t>(2 * num_buys) >= values.size())
    return max_difference_infinite(values);
  ...
  // at i
  const auto state_size = static_cast<std::size_t>(2 * num_buys + 1);
  ...
  for (std::uint32_t i = 0; i < values.size(); ++i) {
    ...
    for (std::uint32_t k = 1, sign = -1; k <= i + 1 && k < state_size;
         ++k, sign *= -1) {
...
// excluding one entry
inline std::int64_t
max_product_all_but_one(std::vector<std::int32_t> const &values) {
  ...
  if (negatives % 2 == 1) {
    ...
    return std::accumulate(
        ...
        product, [state = 1](auto product, auto value) mutable {
    ...
    if (static_cast<std::size_t>(negatives) == values.size()) {
...
max_increasing_subarray(std::vector<std::int32_t> const &values) {
...
apply_permutation(std::vector<std::int32_t> input_output,
  ...
  auto const permute_circle = [&](auto const start) {
    ...
    while (static_cast<std::uint32_t>(pos) != start) {
...
void next_permutation(iterator_type const begin, iterator_type const end) {
  ...
  if (longest_suffix != begin) {
    ...
    auto swap_itr =
        std::find_if(longest_suffix,
                     end, [ref = *(longest_suffix - 1)](
                              auto const value) { return value < ref; }) -
        1;
...
inline std::vector<std::int32_t>
spiral(std::vector<std::vector<std::int32_t>> const &data) {
