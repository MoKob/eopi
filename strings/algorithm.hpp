...
inline void print_justified(std::vector<std::string> const &text,
  ...
  for (std::size_t pos = 0, end = 0; pos < text.size(); pos = end) {
    ...
    } else // print justified
    {
      ...
      std::cout << text[pos];
      for (auto i = 1; static_cast<std::size_t>(i) < n; ++i) {
        // add spaces + additional one if we got a remainder
        for (auto k = 0;
             static_cast<std::size_t>(k) <
             sep + ((static_cast<std::size_t>(i) - 1 < (spaces % (n - 1))) ? 1
                                                                           : 0);
             ++k)
          std::cout << " ";
...
inline std::size_t rabin_karp(const std::string &text,
  ...
  auto const get_hash = [](auto begin, auto const end) {
    std::int64_t hash = 0;
  ...
  auto local_text_hash = get_hash(text.begin(), text.begin() + pattern.size());
  ...
  for (std::size_t pos = pattern.size(); pos < text.size(); ++pos) {
    ...
    if (local_text_hash < 0)
      local_text_hash += modul;
